...
export function LandingEgressIP({ server, language, disabled, addresses, save }: {
  server: Server; language: Language; disabled: boolean; addresses: string[];
  ...
  return <details className="mt-2 text-xs">
    ...
    <form className="mt-2 space-y-2" onSubmit={(event) => { event.preventDefault(); if (!disabled && server.egressSupported && (mode === "auto" || value)) void save(value); }}>
      ...
      {mode === "fixed" ? <><label htmlFor={id}>{copy(language, "服务器本机 IP", "Local server IP")}</label><Input id={id} list={`${id}-addresses`} value={address} onChange={(event) => setAddress(event.target.value)} required placeholder={copy(language, "填写网卡上的 IPv4 或 IPv6", "IPv4 or IPv6 assigned to the server")} disabled={disabled} autoComplete="off" spellCheck={false} /><datalist id={`${id}-addresses`}>{addresses.map((ip) => <option key={ip} value={ip} />)}</datalist><p className="text-muted-foreground">{copy(language, "绑定此地址，不回退其他 IP。IPv6 出口仅能访问 IPv6 目标；NAT 主机填写网卡地址。", "Bind this address without fallback. IPv6 exits reach IPv6 destinations only; on NAT hosts enter the interface address.")}</p></> : null}

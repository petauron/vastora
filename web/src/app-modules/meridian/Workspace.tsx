...
import { LandingManager, LandingNotice, LandingTableRows } from "@/views/LandingControls";
...
export function MeridianWorkspace({ group, language, mutate, onManage, onUpgrade, onClients, showSite }: AppWorkspaceProps) {
  ...
  return <section aria-label="Meridian" className="apps-three-xui flex min-w-0 flex-col gap-3" data-app-workspace={manifest.appKey}>
    <header><h2 className="text-base font-medium">Meridian</h2><p className="mt-1 text-xs text-muted-foreground">{copy(language, `${entries.length} 个线路机 · ${controller ? 1 : 0} 台订阅主机`, `${entries.length} entry nodes · ${controller ? 1 : 0} subscription host`)}</p></header>
    ...
    <Tabs value={page} onValueChange={(value) => { if (typeof value === "string") setPage(value); }}>
      ...
      <TabsContent value="nodes"><Table aria-label={copy(language, "Meridian 节点", "Meridian nodes")} className="apps-instance-table block lg:table lg:table-fixed">
        <TableHeader className="hidden lg:table-header-group"><TableRow><TableHead className="w-[24%]">{copy(language, "节点", "Node")}</TableHead><TableHead className="w-[48%]">{copy(language, "IP 质量与解锁", "IP quality & availability")}</TableHead><TableHead className="w-[12%]">{copy(language, "状态", "Status")}</TableHead><TableHead className="w-[10%]">{copy(language, "入口", "Access")}</TableHead><TableHead className="w-[6%]"><span className="sr-only">{copy(language, "操作", "Actions")}</span></TableHead></TableRow></TableHeader>
        <TableBody className="block lg:table-row-group"><TableRow className="block bg-muted/30 lg:table-row"><TableCell colSpan={5} className="block text-xs lg:table-cell">{copy(language, "线路机", "Entry nodes")} {instances.length}</TableCell></TableRow>
          {instances.map((instance) => {
            ...
            return <TableRow key={instance.application.id} data-application-id={instance.application.id} className="grid grid-cols-2 gap-x-4 gap-y-3 py-4 lg:table-row lg:py-0">
              <TableCell className="col-span-2 min-w-0 p-0 whitespace-normal lg:px-2 lg:py-3"><div className="flex items-center gap-2"><RegionFlag code={service?.regionCode} language={language} /><span className="font-medium">{name}</span></div><p className="mt-1 text-xs text-muted-foreground">{showSite ? instance.siteName : service?.displayName} {service?.protocols?.map((protocol) => protocol.toUpperCase()).join(" · ")}</p></TableCell>
          ...
          <LandingTableRows language={language} search={search} />
        </TableBody>
      </Table></TabsContent>

...
import { AppWindowIcon, ArrowRightLeftIcon, ArrowUpCircleIcon, CheckCircle2Icon, ExternalLinkIcon, Globe2Icon, KeyRoundIcon, PackagePlusIcon, PencilIcon, RadioTowerIcon, RotateCcwIcon, Settings2Icon, ShieldAlertIcon, Trash2Icon, UsersIcon } from "lucide-react";
import { api } from "../api";
import type { AppData, Mutate } from "../App";
import type { AgentView, Application, ApplicationCommand, AppView, Deployment, Publication, PublicationKind, Service, ThreeXUIControllerMigration, ThreeXUIRole } from "../types";
...
export function AppsView({ data, language, mutate }: { data: AppData; language: Language; mutate: Mutate }) {
  ...
  return (
    <section className="flex flex-col gap-7">
      ...
      <DeploymentSheet data={data} editor={deploymentEditor} language={language} onClose={() => setDeploymentEditor(null)} onSubmit={async (agent, app, config, operation, role) => {
        ...
        await mutate(async () => { result = await api.createDeployment(agent.id, app.key, config, operation, false, role); }, messages[operation]);
      ...
      <PublicationSheet data={data} language={language} onClose={() => setPublicationService(null)} onSubmit={async (input) => { await mutate(() => api.createPublication(input), copy(language, "访问入口已创建。", "Access point created.")); setPublicationService(null); }} service={publicationService} />
		...
		<RealityRenameSheet language={language} mutate={mutate} onClose={() => setRealityRenameService(null)} service={realityRenameService} />
...
function InstalledAppCard({ application, app, data, language, onClients, onConfigure, onMigrate, onPublish, onReality, onRenameReality, onSubscription, onUninstall, onUpgrade, mutate }: { application: Application; app?: AppView; data: AppData; language: Language; onClients: () => void; onConfigure: () => void; onMigrate: () => void; onPublish: (service: Service) => void; onReality: () => void; onRenameReality: (service: Service) => void; onSubscription: () => void; onUninstall: () => void; onUpgrade: () => void; mutate: Mutate }) {
  ...
  return <Card>
    ...
    <CardContent className="flex flex-col gap-3">
      {activeChange ? <Alert><Spinner /><AlertTitle>{copy(language, `正在${operationLabel(language, activeChange.operation)}`, `${operationLabel(language, activeChange.operation)} in progress`)}</AlertTitle><AlertDescription>{copy(language, "完成前暂时不能发起其他应用变更。", "Other app changes are unavailable until this finishes.")}</AlertDescription></Alert> : null}
      ...
      {isThreeXUI ? <div className={`grid gap-2 ${isController ? "sm:grid-cols-3" : "sm:grid-cols-1"}`}>
        {isController ? <Button disabled={Boolean(activeChange)} onClick={onClients} size="sm"><UsersIcon data-icon="inline-start" />{copy(language, "管理客户端", "Manage clients")}</Button> : null}
        <Button disabled={Boolean(activeChange) || isWorker && (!nodeReady || syncingNode)} onClick={onReality} size="sm" variant={isController ? "outline" : "default"}><RadioTowerIcon data-icon="inline-start" />{copy(language, "创建 VLESS", "Create VLESS")}</Button>
        {isController && subscriptionService ? <Button disabled={Boolean(activeChange)} onClick={onSubscription} size="sm" variant="outline"><Globe2Icon data-icon="inline-start" />{subscriptionPublication ? copy(language, "公网订阅", "Public subscription") : copy(language, "开启订阅", "Enable subscription")}</Button> : null}
      </div> : null}
      ...
      {!isWorker && visibleServices.length === 0 ? <p className="text-sm text-muted-foreground">{copy(language, "此应用没有可发布的 Web 服务。", "This app has no publishable Web service.")}</p> : null}
		{visibleServices.map((service) => <ServiceRow data={data} key={service.id} language={language} onPublish={() => onPublish(service)} onRename={() => onRenameReality(service)} service={service} mutate={mutate} />)}
...
function ThreeXUIControllerMigrationSheet({ application, data, language, mutate, onClose }: { application: Application | null; data: AppData; language: Language; mutate: Mutate; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (!cancelled) setMigration(next);
        if (!cancelled && next.state === "ready") await mutate(async () => undefined, copy(language, "订阅主机迁移完成，原来的域名和订阅地址保持不变。", "Subscription host moved. Existing domains and subscription URLs were preserved."));
      } catch (pollError) {
...
function ServiceRow({ data, language, service, onPublish, onRename, mutate }: { data: AppData; language: Language; service: Service; onPublish: () => void; onRename: () => void; mutate: Mutate }) {
	...
	return <div className="rounded-xl border p-3"><div className="flex flex-wrap items-center gap-2"><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{service.displayName || service.name}</p>{isReality && service.displayName ? <p className="mt-0.5 truncate text-xs text-muted-foreground">{service.name}</p> : null}<p className="mt-0.5 truncate font-mono text-xs text-muted-foreground">{service.protocol} · {service.endpoint}</p></div>{service.management ? <Badge variant="destructive">{copy(language, "管理页", "Admin")}</Badge> : null}{isReality ? <Button onClick={onRename} size="sm" variant="outline"><PencilIcon data-icon="inline-start" />{copy(language, "重命名", "Rename")}</Button> : null}<Button onClick={onPublish} size="sm" variant="outline"><Globe2Icon data-icon="inline-start" />{copy(language, "添加入口", "Add access")}</Button></div>{publications.length ? <div className="mt-3 flex flex-col gap-2">{publications.map((publication) => <PublicationRow cloudflareReady={cloudflareReady} key={publication.id} language={language} mutate={mutate} publication={publication} />)}</div> : <p className="mt-3 text-xs text-muted-foreground">{copy(language, "仅私有源站，尚未添加访问入口。", "Private origin only; no access point added.")}</p>}</div>;
}

function PublicationRow({ publication, language, mutate, cloudflareReady }: { publication: Publication; language: Language; mutate: Mutate; cloudflareReady: boolean }) {
  const [busyAction, setBusyAction] = useState<"tls" | "verify" | "stop" | null>(null);
  const run = async (action: "tls" | "verify" | "stop", operation: () => Promise<unknown>, success: string) => { setBusyAction(action); try { await mutate(operation, success); } catch { /* The shared notice already explains the failure. */ } finally { setBusyAction(null); } };
  ...
  return <div className="flex flex-col gap-2 rounded-lg bg-muted/60 p-3 text-xs sm:flex-row sm:items-center"><StateBadge value={publication.status} /><div className="min-w-0 flex-1"><p className="truncate font-medium">{publication.hostname}</p><p className="mt-0.5 text-muted-foreground">{publicationKindLabel(language, publication.kind)}{privateWeb ? ` · ${publication.tlsEnabled ? "HTTPS" : "HTTP"}` : ""}</p>{publication.sniHostname ? <p className="mt-0.5 truncate font-mono text-muted-foreground">SNI → {publication.sniHostname}</p> : null}{publication.lastError ? <div className="mt-1"><TechnicalError error={publication.lastError} language={language} /></div> : null}{publication.dnsRecord && publication.dnsProvider !== "cloudflare" ? <code className="mt-1 block break-all text-muted-foreground">{publication.dnsRecord.type} {publication.dnsRecord.name} → {publication.dnsRecord.value}</code> : null}</div><div className="flex flex-wrap items-center gap-2">{privateWeb ? <div aria-busy={busyAction === "tls"} className="flex min-h-9 items-center gap-2 rounded-lg border bg-background px-2.5" title={tlsUnavailable ? copy(language, "连接 Cloudflare 后才能申请可信证书", "Connect Cloudflare to issue a trusted certificate") : undefined}><label className="font-medium" htmlFor={`publication-tls-${publication.id}`} role={busyAction === "tls" ? "status" : undefined}>{tlsStatus}</label>{busyAction === "tls" ? <Spinner aria-hidden="true" /> : null}<Switch aria-label={tlsLabel} checked={publication.tlsEnabled} disabled={busyAction !== null || tlsUnavailable} id={`publication-tls-${publication.id}`} onCheckedChange={(enabled) => void run("tls", () => api.updatePublicationTLS(publication.id, enabled), enabled ? copy(language, "正在启用 HTTPS，入口配置已提交。", "HTTPS is being enabled and the access configuration was submitted.") : copy(language, "HTTPS 已关闭，入口将使用 HTTP。", "HTTPS was turned off; the access point will use HTTP."))} /></div> : null}{publication.accessUrl ? <Button aria-label={copy(language, "打开服务", "Open service")} nativeButton={false} render={<a href={publication.accessUrl} rel="noreferrer" target="_blank" />} size="icon-sm" variant="ghost"><ExternalLinkIcon /></Button> : null}{publication.status !== "ready" ? <Button disabled={busyAction !== null} onClick={() => void run("verify", () => api.verifyPublication(publication.id), copy(language, "入口检查已完成。", "Access point checked."))} size="sm" variant="outline">{busyAction === "verify" ? <Spinner data-icon="inline-start" /> : null}{copy(language, "检查", "Check")}</Button> : null}<Button disabled={busyAction !== null} onClick={() => void run("stop", () => api.stopPublication(publication.id), copy(language, "入口已停止。", "Access point stopped."))} size="sm" variant="ghost">{busyAction === "stop" ? <Spinner data-icon="inline-start" /> : null}{copy(language, "停止", "Stop")}</Button></div></div>;
}

function DeploymentSheet({ data, editor, language, onClose, onSubmit }: { data: AppData; editor: DeploymentEditor; language: Language; onClose: () => void; onSubmit: (agent: AgentView, app: AppView, config: Record<string, string | boolean | number>, operation: "install" | "upgrade" | "configure", role?: ThreeXUIRole) => Promise<void> }) {
...
function RealitySheet({ application, data, language, onClose }: { application: Application | null; data: AppData; language: Language; onClose: () => void }) {
  ...
  const gateways = application ? data.agents.filter((agent) => agent.siteId === application.siteId && agent.connected && agent.capabilities.gateway && agent.networkProfile?.directPublic && agent.networkProfile.enabledKinds.includes("public") && data.sites.some((site) => site.id === application.siteId && site.gatewayNodes.includes(agent.id))).sort((left, right) => Number(right.id === application.nodeId) - Number(left.id === application.nodeId) || left.name.localeCompare(right.name)) : [];
	...
	useEffect(() => {
		if (!application) return;
		setDisplayName(targetAgent?.name ?? copy(language, "VLESS 节点", "VLESS node"));
		setClientName(copy(language, "我的设备", "My device"));
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setBusy(true); setError("");
    try {
		setCommand(await api.createRealityCommand({ applicationId: application.id, displayName, clientName, gatewayNodeId: gatewayID, hostname, dnsProvider, target: target || undefined, sniHostname: sniHostname || undefined }));
    } catch (submitError) {
      setError(userError(language, submitError));
    } finally {
      setBusy(false);
...
function RealityRenameSheet({ language, mutate, onClose, service }: { language: Language; mutate: Mutate; onClose: () => void; service: Service | null }) {
	...
	useEffect(() => {
		if (!service) return;
		setDisplayName(service.displayName || service.name);
		setCommand(null);
		setBusy(false);
		setError("");
	}, [service?.id]);
	useEffect(() => {
		...
		const poll = async () => {
			try {
				...
				if (next.state === "succeeded") {
					await mutate(async () => undefined, copy(language, `订阅节点已重命名为“${next.displayName ?? displayName}”。`, `Subscription node renamed to “${next.displayName ?? displayName}”.`));
		...
		return () => { cancelled = true; window.clearTimeout(timer); };
	}, [command?.id, command?.state, language, mutate]);
	const submit = async (event: FormEvent<HTMLFormElement>) => {
		event.preventDefault();
		if (!service) return;
		setBusy(true);
		setError("");
		try {
			setCommand(await api.renameRealityCommand(service.id, displayName.trim()));
		} catch (submitError) {
			setError(userError(language, submitError));
		} finally {
			setBusy(false);
	...
	return <Sheet onOpenChange={(next) => { if (!next) onClose(); }} open={Boolean(service)}><SheetContent className="sm:max-w-md"><SheetHeader><SheetTitle>{copy(language, "重命名订阅节点", "Rename subscription node")}</SheetTitle><SheetDescription>{copy(language, "新名称会同步到 3x-ui，并出现在 VLESS 链接和统一订阅的节点列表中。连接域名、密钥和客户端不会改变。", "The new name is synced to 3x-ui and appears in VLESS links and shared subscriptions. Hostnames, keys, and clients stay unchanged.")}</SheetDescription></SheetHeader>{command ? <div aria-live="polite" className="flex flex-1 flex-col gap-4 px-4"><Alert variant={command.state === "failed" ? "destructive" : "default"}>{active ? <Spinner /> : command.state === "succeeded" ? <CheckCircle2Icon /> : <ShieldAlertIcon />}<AlertTitle>{active ? copy(language, "正在同步名称…", "Syncing the name…") : command.state === "succeeded" ? copy(language, "名称已更新", "Name updated") : copy(language, "重命名失败", "Rename failed")}</AlertTitle><AlertDescription>{active ? copy(language, "等待订阅主机更新 3x-ui 入站。", "Waiting for the subscription controller to update the 3x-ui inbound.") : command.state === "succeeded" ? copy(language, `现在显示为“${command.displayName ?? displayName}”。客户端刷新订阅后会看到新名称。`, `It is now shown as “${command.displayName ?? displayName}”. Clients see it after refreshing the subscription.`) : command.error}</AlertDescription></Alert>{error ? <FieldError role="alert">{error}</FieldError> : null}</div> : <form className="flex min-h-0 flex-1 flex-col" onSubmit={(event) => void submit(event)}><div className="flex-1 px-4"><FieldGroup><Field><FieldLabel htmlFor="reality-rename-display-name">{copy(language, "订阅节点名称", "Subscription node name")}</FieldLabel><Input autoFocus id="reality-rename-display-name" maxLength={64} onChange={(event) => setDisplayName(event.target.value)} required value={displayName} /><FieldDescription>{copy(language, "建议使用容易识别的位置或线路名，例如“洛杉矶 9929”。", "Use a recognizable location or route, for example “Los Angeles 9929”.")}</FieldDescription></Field>{error ? <FieldError role="alert">{error}</FieldError> : null}</FieldGroup></div><SheetFooter><Button onClick={onClose} type="button" variant="outline">{copy(language, "取消", "Cancel")}</Button><Button disabled={busy || !displayName.trim() || displayName.trim() === (service?.displayName || service?.name)} type="submit">{busy ? <Spinner data-icon="inline-start" /> : <PencilIcon data-icon="inline-start" />}{copy(language, "保存名称", "Save name")}</Button></SheetFooter></form>}<SheetFooter>{command ? <Button disabled={active} onClick={onClose}>{copy(language, "完成", "Done")}</Button> : null}</SheetFooter></SheetContent></Sheet>;
}

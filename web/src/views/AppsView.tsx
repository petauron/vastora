...
import { AppWindowIcon, ArrowRightLeftIcon, ArrowUpCircleIcon, CheckCircle2Icon, ExternalLinkIcon, Globe2Icon, KeyRoundIcon, PackagePlusIcon, RadioTowerIcon, RotateCcwIcon, Settings2Icon, ShieldAlertIcon, Trash2Icon, UsersIcon } from "lucide-react";
import { api } from "../api";
...
function InstalledAppCard({ application, app, data, language, onClients, onConfigure, onMigrate, onPublish, onReality, onSubscription, onUninstall, onUpgrade, mutate }: { application: Application; app?: AppView; data: AppData; language: Language; onClients: () => void; onConfigure: () => void; onMigrate: () => void; onPublish: (service: Service) => void; onReality: () => void; onSubscription: () => void; onUninstall: () => void; onUpgrade: () => void; mutate: Mutate }) {
  ...
  return <Card>
    ...
    <CardContent className="flex flex-col gap-3">
      ...
      {isController ? <p className="text-sm text-muted-foreground">{copy(language, `这是当前位置唯一的订阅主机，统一管理 ${activeWorkers.length} 个 VLESS 节点。`, `This is the location's only subscription controller and manages ${activeWorkers.length} VLESS node(s).`)}</p> : null}
      ...
      {visibleServices.map((service) => <ServiceRow data={data} key={service.id} language={language} onPublish={() => onPublish(service)} service={service} mutate={mutate} />)}

...
function SourceCard({ language, mutate, source }: { language: Language; mutate: Mutate; source: CatalogSource }) {
  ...
  const trustDescription = source.status === "pending" ? copy(language, "请刷新目录，完成首次验证后即可安装。", "Refresh the catalog to verify it before installing.") : source.status === "expired" ? copy(language, "目录需要重新验证，请刷新后再安装或升级。已安装应用不受影响。", "Refresh the catalog before installing or upgrading. Installed apps are unaffected.") : source.status === "stale" ? copy(language, "正在继续使用最后一次验证通过的缓存。", "Continuing to use the last verified cache.") : source.status === "failed" ? copy(language, "目录暂不可用于安装，请重新刷新。", "The catalog is unavailable for installation. Refresh to retry.") : source.status === "disabled" ? copy(language, "不会自动刷新，也不会提供新的安装。", "Automatic refresh and new installs are paused.") : copy(language, "目录签名与内容已验证。", "Catalog signature and content are verified.");
  const sourceName = official ? copy(language, "Vastora 官方目录", "Vastora Official Catalog") : source.displayName;
  return <><Card><CardHeader><CardTitle className="flex min-w-0 flex-wrap items-center gap-2"><DatabaseIcon className="shrink-0" /><span className="truncate" title={sourceName}>{sourceName}</span><Badge aria-label={copy(language, "目录身份", "Catalog identity")} variant="outline">{official ? copy(language, "官方目录", "Official catalog") : copy(language, "第三方目录", "Third-party catalog")}</Badge></CardTitle><CardDescription className="truncate" title={source.url}>{source.url}</CardDescription><CardAction><StateBadge language={language} value={source.status} /></CardAction></CardHeader><CardContent><p className="mb-3 text-sm text-muted-foreground">{trustDescription}</p><dl className="grid gap-3 text-sm sm:grid-cols-3">{official ? <><div><dt className="text-muted-foreground">{copy(language, "目录修订", "Catalog revision")}</dt><dd className="mt-1">{source.catalogRevision ?? "—"}</dd></div><div><dt className="text-muted-foreground">{copy(language, "有效期至", "Valid until")}</dt><dd className="mt-1">{formatDate(language, source.expiresAt)}</dd></div></> : null}<div><dt className="text-muted-foreground">{copy(language, "上次验证", "Last verified")}</dt><dd className="mt-1">{formatDate(language, source.fetchedAt)}</dd></div><div><dt className="text-muted-foreground">{copy(language, "上次检查", "Last checked")}</dt><dd className="mt-1">{formatDate(language, source.checkedAt)}</dd></div><div><dt className="text-muted-foreground">{copy(language, "刷新间隔", "Refresh interval")}</dt><dd className="mt-1">{Math.round(source.refreshIntervalSeconds / 60)} min</dd></div></dl>{source.lastError ? <div className="mt-3"><TechnicalError error={source.lastError} language={language} /></div> : null}</CardContent><CardFooter className="flex-wrap justify-end gap-2">{!official ? <><Button className="min-h-11" disabled={busy} onClick={() => setEditing(true)} size="sm" variant="outline"><PencilIcon data-icon="inline-start" />{copy(language, "编辑", "Edit")}</Button><Button className="min-h-11" disabled={busy} onClick={() => void toggle()} size="sm" variant="outline"><PowerIcon data-icon="inline-start" />{source.enabled ? copy(language, "停用", "Disable") : copy(language, "启用", "Enable")}</Button><Button className="min-h-11" disabled={busy} onClick={() => void remove()} size="sm" variant="destructive"><Trash2Icon data-icon="inline-start" />{copy(language, "删除", "Delete")}</Button></> : null}<Button className="min-h-11" disabled={busy || !source.enabled} onClick={() => void refresh()} size="sm" variant="outline">{busy ? <Spinner data-icon="inline-start" /> : <RefreshCwIcon data-icon="inline-start" />}{copy(language, "刷新", "Refresh")}</Button></CardFooter></Card>{!official && editing ? <SourceEditSheet language={language} mutate={mutate} onClose={() => setEditing(false)} open source={source} /> : null}</>;
}

...
function PasswordSheet({ language, onClose, open }: { language: Language; onClose: () => void; open: boolean }) {
  ...
  return <Sheet onOpenChange={(next) => { if (!next) close(); }} open={open}><SheetContent><SheetHeader><SheetTitle>{copy(language, "修改管理员密码", "Change administrator password")}</SheetTitle><SheetDescription>{copy(language, "修改后会自动退出其他浏览器中的登录会话。", "Changing it automatically signs out other browser sessions.")}</SheetDescription></SheetHeader><form className="flex flex-1 flex-col" onSubmit={(event) => void submit(event)}><div className="flex-1 px-4"><FieldGroup><Field><FieldLabel htmlFor="current-password">{copy(language, "当前密码", "Current password")}</FieldLabel><Input autoComplete="current-password" id="current-password" onChange={(event) => setCurrentPassword(event.target.value)} required type="password" value={currentPassword} /></Field><Field><FieldLabel htmlFor="new-password">{copy(language, "新密码", "New password")}</FieldLabel><Input autoComplete="new-password" id="new-password" minLength={12} onChange={(event) => setNewPassword(event.target.value)} required type="password" value={newPassword} /><FieldDescription>{copy(language, "至少 12 个字符。", "At least 12 characters.")}</FieldDescription></Field><Field data-invalid={Boolean(error)}><FieldLabel htmlFor="confirm-new-password">{copy(language, "再次输入新密码", "Enter new password again")}</FieldLabel><Input aria-invalid={Boolean(error)} autoComplete="new-password" id="confirm-new-password" minLength={12} onChange={(event) => setConfirmation(event.target.value)} required type="password" value={confirmation} />{error ? <FieldError>{error}</FieldError> : null}</Field></FieldGroup></div><SheetFooter><Button onClick={close} type="button" variant="outline">{copy(language, "取消", "Cancel")}</Button><Button disabled={busy || !currentPassword || newPassword.length < 12 || newPassword !== confirmation} type="submit">{busy ? <Spinner data-icon="inline-start" /> : <KeyRoundIcon data-icon="inline-start" />}{copy(language, "修改密码", "Change password")}</Button></SheetFooter></form></SheetContent></Sheet>;
}

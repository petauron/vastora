      ...
      <div className="flex min-h-0 flex-1 flex-col gap-6 overflow-y-auto px-4 pb-4">
        ...
        <Tabs defaultValue="overview" className="min-w-0">
          ...
          <TabsContent value="quality" className="space-y-5 pt-3">{report ? <>
            ...
            <section className="space-y-2" aria-label={copy(language, "流媒体与 AI 解锁", "Streaming and AI availability")}><h3 className="font-medium">{copy(language, "流媒体与 AI 解锁", "Streaming and AI availability")}</h3><Table><TableHeader><TableRow><TableHead>{copy(language, "服务", "Service")}</TableHead><TableHead>{copy(language, "结果", "Result")}</TableHead><TableHead>{copy(language, "地区", "Region")}</TableHead></TableRow></TableHeader><TableBody>{report.services.map((service) => <TableRow key={service.name}><TableCell className="whitespace-normal">{service.name === "AmazonPrimeVideo" ? "Prime Video" : service.name === "DisneyPlus" ? "Disney+" : service.name}</TableCell><TableCell className={cn("whitespace-normal", service.status.toLowerCase() === "yes" ? "text-latency-fast" : service.status.toLowerCase() === "no" ? "text-destructive" : "text-muted-foreground")}>{unlockLabel(language, service.status)}{service.type && service.type !== "null" ? <span className="block text-xs text-muted-foreground">{service.type === "Native" ? copy(language, "原生", "Native") : service.type}</span> : null}</TableCell><TableCell><span className="inline-flex gap-1"><RegionFlag code={service.regionCode} language={language} />{service.regionCode ?? "—"}</span></TableCell></TableRow>)}{!report.services.length ? <TableRow><TableCell colSpan={3}>{copy(language, "暂无解锁结果", "No availability results")}</TableCell></TableRow> : null}</TableBody></Table></section>

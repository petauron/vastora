...
import { api } from "../api";
import { actionKind, actionMessage } from "./ActivityView";
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          {data.sites.map((site) => {
            ...
            return <Card key={site.id}><CardHeader><CardTitle>{site.name}</CardTitle><CardDescription>{site.description || copy(language, "一个网络位置", "A network location")}</CardDescription><CardAction><Button aria-label={copy(language, "编辑位置", "Edit location")} onClick={() => { setEditingSite(site); setSiteEditorOpen(true); }} size="icon-sm" variant="ghost"><PencilIcon /></Button></CardAction></CardHeader><CardContent><dl className="grid grid-cols-2 gap-4 text-sm"><div><dt className="text-muted-foreground">{copy(language, "节点", "Nodes")}</dt><dd className="mt-1 font-medium">{agents.length}</dd></div><div><dt className="text-muted-foreground">{copy(language, "网关", "Gateways")}</dt><dd className="mt-1 font-medium">{site.gatewayNodes.length}</dd></div><div className="col-span-2"><dt className="text-muted-foreground">{copy(language, "默认域名", "Default domain")}</dt><dd className="mt-1 truncate font-mono text-xs">{site.domainSuffix || copy(language, "未设置", "Not set")}</dd></div></dl></CardContent><CardFooter className="justify-between"><StateBadge value={site.gatewayStatus} /><Button onClick={() => onNavigate("network")} size="sm" variant="ghost">{copy(language, "查看网络", "View network")}<ArrowRightIcon data-icon="inline-end" /></Button></CardFooter></Card>;
          })}
        </div>
      </div>
...
function SetupGuide({ activeAgents, language, needsNetwork, onNavigate, runningApps }: { activeAgents: number; language: Language; needsNetwork: number; onNavigate: (screen: Screen) => void; runningApps: number }) {
  const steps = [
    { done: true, title: copy(language, "管理员账号已创建", "Administrator created"), description: copy(language, "Center 已可以安全登录。", "Center is ready for secure sign-in."), screen: "home" as Screen, action: "" },
...
function SiteEditor({ agents, language, open, site, onClose, onSave }: { agents: AgentView[]; language: Language; open: boolean; site: Site | null; onClose: () => void; onSave: (site: Site | null, input: { name: string; code: string; description: string; domainSuffix: string; gatewayNodes: string[] }) => Promise<void> }) {
  ...
  useEffect(() => {
    if (!open) return;
    setName(site?.name ?? ""); setCode(site?.code ?? `site-${crypto.randomUUID().slice(0, 8)}`); setDescription(site?.description ?? ""); setDomainSuffix(site?.domainSuffix ?? ""); setGatewayNodes(site?.gatewayNodes ?? []); setError("");
  }, [open, site]);

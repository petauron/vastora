...
function SiteEditor({ agents, language, open, site, onClose, onSave }: { agents: AgentView[]; language: Language; open: boolean; site: Site | null; onClose: () => void; onSave: (site: Site | null, input: SiteInput) => Promise<void> }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try { await onSave(site, { name, code, description, timezone, domainSuffix, gatewayNodes }); } catch (submitError) { setError(submitError instanceof Error ? submitError.message : "Request failed"); } finally { setBusy(false); }
  };

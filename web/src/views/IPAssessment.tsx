...
export function AssessmentSummary({ language, assessment, report, checkedAt }: { language: Language; assessment?: IPQualityAssessment; report?: IPQualityReport; checkedAt?: string }) {
  ...
  return <section aria-label={copy(language, "出口适用评分", "Exit suitability assessment")} className="flex flex-col gap-2">
    ...
    {expanded ? <div id={id} className="flex flex-col gap-2 rounded-lg border p-3 text-xs">
      <p className="font-medium">{copy(language, "Meridian 评分 v1 · 出口适用性", "Meridian score v1 · Exit suitability")}</p>
      ...
      <p className="text-muted-foreground">{copy(language, "产品选机规则，不是行业标准或通过概率。暂评区间表示缺失数据的可能贡献，不参与正式排名。", "Product selection rules, not an industry standard or probability. Provisional ranges represent missing evidence and are excluded from formal ranking.")}</p>
    </div> : null}
  </section>;
}

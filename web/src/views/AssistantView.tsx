          ...
          {conversation ? <>
			...
			<CardFooter>
			  <form className="flex w-full min-w-0 items-end gap-2" onSubmit={(event) => void submit(event)}>
				<div className="min-w-0 flex-1"><Textarea aria-describedby="assistant-message-security" aria-label={copy(language, "发送给集群助手的消息", "Message to the cluster assistant")} autoComplete="off" className="max-h-36 min-h-11 resize-y" disabled={!providerReady || busy || Boolean(activeRun)} maxLength={8000} onChange={(event) => setContent(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} placeholder={copy(language, "例如：列出离线节点、安装 CPA，或轮换 CPA 客户端密钥", "For example: list offline nodes, install CPA, or rotate a CPA client key")} value={content} /><p className="mt-1 text-xs text-muted-foreground" id="assistant-message-security">{copy(language, "不要粘贴密码、密钥或 Token，请使用专用凭据表单。疑似凭据会被拦截，但无法识别全部秘密；通过检查的消息会保存并发送给已配置的模型服务。", "Do not paste passwords, keys, or tokens; use dedicated credential forms. Suspected credentials are blocked, but not every secret can be detected. Accepted messages are saved and sent to your configured model provider.")}</p></div>

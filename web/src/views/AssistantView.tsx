...
function ProposalCard({ busy, language, onAction, proposal }: { busy: boolean; language: Language; onAction: (action: "approve" | "reject" | "apply") => void; proposal: AssistantProposal }) {
    ...
    {!isCredentialRotation && proposal.summary.permissionDetails?.length ? <CardContent>{proposal.summary.permissionDetails.map((detail) => <p key={detail} className="break-all font-mono text-xs">{detail}</p>)}</CardContent> : null}
    ...
    {!isCredentialRotation ? <CardContent><p className="text-sm">{copy(language, "配方修订", "Recipe revision")}: r{proposal.summary.packageRevision ?? "—"}</p><p className="mt-2 text-sm">{copy(language, "批准后授权的权限", "Permissions granted by approval")}: {proposal.summary.authorizedCapabilities?.join(", ") || copy(language, "无额外权限", "No additional permissions")}</p></CardContent> : null}

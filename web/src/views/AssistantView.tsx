        ...
        <Card className="min-h-[34rem] min-w-0 lg:max-h-[44rem]">
          {conversation ? <>
            ...
            <CardContent className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto" aria-live="polite">
              {conversation.messages.length === 0 && conversation.proposals.length === 0 ? <Empty className="my-auto"><EmptyHeader><EmptyMedia variant="icon"><SparklesIcon /></EmptyMedia><EmptyTitle>{copy(language, "询问集群状态或安装应用", "Ask about the cluster or install an app")}</EmptyTitle><EmptyDescription>{copy(language, "助手可读取节点、站点、目录、应用和任务状态。任何安装都会先显示确定的审批提案。", "The assistant can read nodes, sites, catalogs, applications, and task state. Every install first appears as an exact approval proposal.")}</EmptyDescription></EmptyHeader></Empty> : null}
            ...
            <CardFooter>
              <form className="flex w-full min-w-0 items-end gap-2" onSubmit={(event) => void submit(event)}>
                <Textarea aria-label={copy(language, "发送给集群助手的消息", "Message to the cluster assistant")} className="max-h-36 min-h-11 resize-y" disabled={!providerReady || busy || Boolean(activeRun)} maxLength={8000} onChange={(event) => setContent(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} placeholder={copy(language, "例如：列出离线节点，或在上海节点安装 CPA", "For example: list offline nodes, or install CPA on the Shanghai node")} value={content} />

    ...
    <section className="flex flex-col gap-7">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent><p className="text-sm text-muted-foreground">{headscale.status === "configured" ? copy(language, "安全私网服务已准备好。", "The secure private network is ready.") : copy(language, "尚未设置安全私网。", "The secure private network is not set up.")}</p>{headscale.mode === "builtin" && headscale.credentialExpiresAt ? <p className="mt-2 text-xs text-muted-foreground">{copy(language, "API Key 自动轮换；当前有效期至 ", "API key rotation is automatic; current expiry: ")}{formatDate(language, headscale.credentialExpiresAt)}</p> : null}{headscale.lastError ? <Alert className="mt-3" variant="destructive"><AlertTitle>{copy(language, "Headscale 维护需要重试", "Headscale maintenance needs a retry")}</AlertTitle><AlertDescription>{headscale.lastError}</AlertDescription></Alert> : null}<details className="mt-3 text-xs text-muted-foreground"><summary className="cursor-pointer font-medium text-foreground">{copy(language, "技术信息", "Technical details")}</summary><p className="mt-2 break-all">Headscale · {headscale.mode ?? "—"} · {headscale.endpoint ?? "—"}{headscale.credentialStatus ? ` · key ${headscale.credentialStatus}` : ""}</p></details></CardContent>
      ...
      <div className="flex flex-col gap-4">
        ...
        {centerRemoteAccess ? <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><ShieldCheckIcon />{copy(language, "Center 远程备用入口", "Center remote fallback")}</CardTitle><CardDescription>{copy(language, "安全私网不可用时，通过 Cloudflare Access 登录专用的一层 HTTPS 域名。", "Sign in through a dedicated first-level HTTPS hostname when the secure private network is unavailable.")}</CardDescription><CardAction><StateBadge value={centerRemoteAccess.status} /></CardAction></CardHeader>
      ...
      <NetworkProfileSheet agent={profileAgent} language={language} onClose={() => setProfileAgent(null)} onDetect={async (agent) => { let profile: NetworkProfile | undefined; await mutate(async () => { profile = (await api.detectAgentPublicEntry(agent.id)).profile; }, copy(language, "已自动检测并启用公网入口。", "The public entry was detected and enabled.")); return profile!; }} onSave={async (agent, profile) => { await mutate(() => api.confirmNetworkProfile(agent.id, profile), copy(language, "节点网络已确认。", "Node network confirmed.")); setProfileAgent(null); }} />
...
function publicEntryDetectionError(language: Language, error: unknown) {
  const detail = error instanceof Error ? error.message : typeof error === "string" ? error : "";
  const normalized = detail.toLowerCase();
  if (normalized.includes("active agent not found")) return copy(language, "节点当前不在线，请等待 Agent 重新连接后再检测。", "The node is offline. Wait for the Agent to reconnect, then detect again.");
  if (normalized.includes("gateway-capable")) return copy(language, "这个节点没有 Gateway 能力，不能承接公网网页流量。", "This node is not Gateway-capable and cannot receive public web traffic.");
  if (normalized.includes("gateway") && normalized.includes("healthy")) return copy(language, "Gateway 尚未就绪，请等待节点恢复健康后再检测。", "The Gateway is not ready. Wait for the node to become healthy, then detect again.");
  if (normalized.includes("only for the agent running on this center host") || normalized.includes("share this center host's secure-private address")) return copy(language, "自动检测只适用于与 Center 安装在同一台主机上的 Agent。", "Automatic detection is available only for the Agent installed on the Center host.");
  if (normalized.includes("verified center public mapping") || normalized.includes("public entry verification") || normalized.includes("verify the public ports") || normalized.includes("external verification")) return copy(language, "尚无可复用的公网入站验证，请先确认云防火墙已放行 80/443，并重新完成 Center 公网入口检查。", "No reusable public-ingress verification is available. Allow ports 80/443 in the cloud firewall and verify the Center public entry again.");
  if (normalized.includes("detected public address changed")) return copy(language, "当前公网 IP 与安装时记录不一致，请重新检查 Center 公网入口。", "The current public IP differs from the setup record. Verify the Center public entry again.");
  if (normalized.includes("receiving address is no longer assigned")) return copy(language, "原先验证的本机接收地址已不存在，请等待 Agent 刷新网卡地址后重试。", "The verified local receiving address is no longer assigned. Wait for the Agent to refresh its addresses, then retry.");
  if (normalized.includes("installation cannot detect a public entry")) return copy(language, "当前 Center 安装未提供受管 Gateway 检测能力，无法自动确认云公网入口。", "This Center installation does not provide managed Gateway detection, so it cannot verify a cloud public entry automatically.");
  return userError(language, error);
}

function NetworkProfileSheet({ agent, language, onClose, onDetect, onSave }: { agent: AgentView | null; language: Language; onClose: () => void; onDetect: (agent: AgentView) => Promise<NetworkProfile>; onSave: (agent: AgentView, profile: NetworkProfile) => Promise<void> }) {
  ...
  useEffect(() => { if (agent) { setProfile(suggestedProfile(agent)); setCustomized(Boolean(agent.networkProfile)); setDetected(false); setError(""); } }, [agent]);
  ...
  const detectPublicEntry = async () => {
    if (!agent) return;
    setDetecting(true); setDetected(false); setError("");
    try { setProfile(await onDetect(agent)); setCustomized(false); setDetected(true); } catch (detectError) { setError(publicEntryDetectionError(language, detectError)); } finally { setDetecting(false); }
  };
  return (
            ...
            <FieldGroup>
              ...
              <details className="rounded-xl border p-3">
                ...
                <div className="mt-4 flex flex-col gap-5">
                  <Field><FieldLabel htmlFor="service-address">{copy(language, "应用地址", "App address")}</FieldLabel><SelectControl id="service-address" onValueChange={(value) => updateProfile((current) => ({ ...current, serviceAddress: value }))} options={[{ value: "127.0.0.1", label: `127.0.0.1 — ${copy(language, "仅本机", "this node only")}` }, ...(agent?.networkCandidates.filter((candidate) => candidate.kind !== "public").map((candidate) => ({ value: candidate.address, label: `${candidate.address} — ${candidate.interface}` })) ?? [])]} value={profile.serviceAddress} /><FieldDescription>{copy(language, "安装应用后若要修改，需要先停止该节点上的应用。", "Stop apps on this node before changing it later.")}</FieldDescription></Field>
                  ...
                  <FieldSet><FieldLegend>{copy(language, "公网地址", "Public address")}</FieldLegend><FieldDescription>{copy(language, "网卡公网地址可直接确认；云 NAT 地址必须由 Vastora 匹配本机接收地址，并复用安装时的外部入站验证。", "An on-interface public address can be confirmed directly. A cloud NAT address must be matched to this host and reuse the external ingress verification completed during setup.")}</FieldDescription>{profile.directPublic && profile.publicMode === "nat" ? <><Alert><ShieldCheckIcon /><AlertTitle>{copy(language, "云 NAT 公网入口已验证", "Cloud NAT public entry verified")}</AlertTitle><AlertDescription><span className="break-all font-mono">{profile.publicAddress} → {profile.publicBindAddress}</span>{profile.publicVerifiedAt ? <span className="mt-1 block text-xs">{copy(language, "外部验证时间：", "Externally verified: ")}{formatDate(language, profile.publicVerifiedAt)}</span> : null}</AlertDescription></Alert><Field orientation="horizontal"><FieldLabel htmlFor="nat-public-enabled">{copy(language, "允许公网网页发布", "Allow public web publishing")}</FieldLabel><Switch checked id="nat-public-enabled" onCheckedChange={(checked) => { if (!checked) updateProfile((current) => ({ ...current, enabledKinds: current.enabledKinds.filter((value) => value !== "public"), directPublic: false, publicAddress: "", publicBindAddress: "", publicMode: undefined, publicVerifiedAt: undefined })); }} /></Field></> : <><Field orientation="horizontal" data-disabled={grouped.public.length === 0}><FieldLabel htmlFor="direct-public">{copy(language, "允许直接公网发布", "Allow direct public access")}</FieldLabel><Switch checked={profile.directPublic} disabled={grouped.public.length === 0} id="direct-public" onCheckedChange={(checked) => { setCustomized(true); const address = grouped.public[0]?.address ?? ""; setProfile((current) => ({ ...current, enabledKinds: checked ? [...new Set([...current.enabledKinds, "public" as NetworkKind])] : current.enabledKinds.filter((value) => value !== "public"), directPublic: checked, publicAddress: checked ? address : "", publicBindAddress: checked ? address : "", publicMode: checked ? "direct" : undefined, publicVerifiedAt: undefined })); }} /></Field>{profile.directPublic ? <Field><FieldLabel htmlFor="public-address">{copy(language, "使用地址", "Use address")}</FieldLabel><SelectControl id="public-address" onValueChange={(value) => updateProfile((current) => ({ ...current, publicAddress: value, publicBindAddress: value, publicMode: "direct" }))} options={grouped.public.map((candidate) => ({ value: candidate.address, label: `${candidate.address} — ${candidate.interface}` }))} value={profile.publicAddress ?? ""} /></Field> : null}{grouped.public.length === 0 ? <div className="rounded-xl border border-dashed p-3"><p className="text-xs leading-5 text-muted-foreground">{copy(language, "如果这是与 Center 同机的云服务器，Vastora 可以自动核对公网 IP、本机接收地址、安装时的外部验证记录和 Gateway 状态。不会修改云防火墙。", "For a cloud server on the Center host, Vastora can automatically match the public IP, local receiving address, setup-time external verification, and Gateway health. It never changes the cloud firewall.")}</p><Button className="mt-3 w-full" disabled={detecting || !agent?.capabilities.gateway} onClick={() => void detectPublicEntry()} type="button" variant="outline">{detecting ? <Spinner data-icon="inline-start" /> : <Globe2Icon data-icon="inline-start" />}{detecting ? copy(language, "正在检测…", "Detecting…") : copy(language, "自动检测云公网入口", "Detect cloud public entry")}</Button></div> : null}</>}</FieldSet>
                  <details className="rounded-lg bg-muted/40 p-3 text-xs text-muted-foreground"><summary className="cursor-pointer font-medium text-foreground">{copy(language, "传输技术说明", "Transport details")}</summary><p className="mt-2 leading-5">{copy(language, "局域网和安全私网入口默认可使用 HTTP；连接 Cloudflare 后，也可为私网域名启用浏览器可信 HTTPS，而不把服务暴露到公网。公网网页始终使用 HTTPS。", "Local and secure private access can use HTTP by default. After connecting Cloudflare, private hostnames can use browser-trusted HTTPS without exposing the service publicly. Public websites always use HTTPS.")}</p></details>
                </div>
              </details>
              {detected ? <Alert><ShieldCheckIcon /><AlertTitle>{copy(language, "公网入口已启用", "Public entry enabled")}</AlertTitle><AlertDescription>{copy(language, "检测结果已经保存；公网网页现在可以选择“公网直连”。原始 TCP/UDP 端口仍要求网卡直配公网地址。", "The result is saved. Public web services can now use Direct public. Raw TCP/UDP ports still require an on-interface public address.")}</AlertDescription></Alert> : null}
              {error ? <FieldError role="alert">{error}</FieldError> : null}
            </FieldGroup>
          </div>
          <SheetFooter><Button onClick={onClose} type="button" variant="outline">{copy(language, "取消", "Cancel")}</Button><Button disabled={busy || detecting} type="submit">{busy ? <Spinner data-icon="inline-start" /> : null}{customized ? copy(language, "保存配置", "Save setup") : copy(language, "确认并关闭", "Confirm and close")}</Button></SheetFooter>
        </form>
      </SheetContent>
    </Sheet>
  );

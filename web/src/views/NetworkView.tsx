        ...
        <form className="flex min-h-0 flex-1 flex-col" onSubmit={(event) => void submit(event)}>
          <div className="flex-1 overflow-y-auto px-4">
            <FieldGroup>
              ...
              <FieldSet><FieldLegend>{copy(language, "公网网页", "Public web access")}</FieldLegend><FieldDescription>{copy(language, "Agent 每次启动时检测一次公网出口；这里只保存公网地址到本机接收地址的映射，不会修改云防火墙或保证端口已可达。", "The Agent detects its public egress once at startup. This only saves the mapping to the local receiving address; it does not change the cloud firewall or prove that ports are reachable.")}</FieldDescription><Field orientation="horizontal" data-disabled={!profile.directPublic && !publicEgressReady}><FieldLabel htmlFor="public-web-enabled"><span>{copy(language, "允许公网网页发布", "Allow public web publishing")}</span><span className="text-xs font-normal text-muted-foreground">{publicEgress ? `${publicEgress.address} → ${publicEgress.bindAddress} · ${publicEgress.mode === "nat" ? copy(language, "云 NAT", "Cloud NAT") : copy(language, "公网网卡", "Public interface")}` : copy(language, "等待 Agent 启动检测公网出口", "Waiting for the Agent startup egress check")}</span></FieldLabel><Switch checked={profile.directPublic} disabled={!profile.directPublic && !publicEgressReady} id="public-web-enabled" onCheckedChange={togglePublic} /></Field>{publicEgress ? <FieldDescription>{copy(language, "本次 Agent 启动检测：", "Detected at Agent startup: ")}{formatDate(language, publicEgress.observedAt)}{!agent?.connected ? copy(language, "（Agent 当前离线，重新启动后会再次检测）", " (Agent is offline; it will detect again after restart)") : ""}</FieldDescription> : null}</FieldSet>

...
import { CableIcon, CloudIcon, CopyIcon, Globe2Icon, KeyRoundIcon, NetworkIcon, RouterIcon, ServerIcon, TerminalIcon } from "lucide-react";
import { api } from "../api";
...
function HeadscaleSheet({ integration, language, open, onClose, onSave }: { integration: Integration; language: Language; open: boolean; onClose: () => void; onSave: (input: { mode: "builtin" | "external"; url: string; apiKey: string }) => Promise<void> }) {
  ...
  const keyInvalid = apiKey !== "" && apiKey.length < 20;
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); setURLTouched(true); setError("");
    if (!validHTTPSURL(url)) return;
    setBusy(true);
    try { await onSave({ mode, url, apiKey }); } catch (submitError) { setError(submitError instanceof Error ? submitError.message : "Request failed"); } finally { setBusy(false); }
  };
  const apiKeyCommand = "docker compose exec headscale headscale apikeys create --expiration 365d";
  return <Sheet onOpenChange={(next) => { if (!next) onClose(); }} open={open}><SheetContent className="sm:max-w-lg"><SheetHeader><SheetTitle>{copy(language, "连接 Headscale", "Connect Headscale")}</SheetTitle><SheetDescription>{copy(language, "按顺序填写，Center 会先验证连接再保存。已有密钥无需重复输入。", "Follow the steps in order. Center verifies the connection before saving, and an existing key need not be re-entered.")}</SheetDescription></SheetHeader><form className="flex min-h-0 flex-1 flex-col" onSubmit={(event) => void submit(event)}><div className="flex-1 overflow-y-auto px-4"><FieldGroup><Field><FieldLabel htmlFor="headscale-mode">1. {copy(language, "选择类型", "Choose type")}</FieldLabel><NativeSelect id="headscale-mode" onChange={(event) => setMode(event.target.value as "builtin" | "external")} value={mode}><option value="builtin">{copy(language, "Center 部署栈内置", "Built into Center stack")}</option><option value="external">{copy(language, "现有 Headscale", "Existing Headscale")}</option></NativeSelect></Field>{mode === "builtin" ? <Alert><TerminalIcon /><AlertTitle>{copy(language, "2. 创建一次 API Key", "2. Create one API key")}</AlertTitle><AlertDescription><p>{copy(language, "在 Center 部署目录运行下面的命令，然后复制输出的 Key。", "Run this in the Center deployment directory, then copy the returned key.")}</p><div className="relative mt-3"><code className="block break-all rounded-lg bg-muted p-3 pr-12 text-xs">{apiKeyCommand}</code><Button aria-label={copy(language, "复制命令", "Copy command")} className="absolute right-1.5 top-1.5" onClick={() => void navigator.clipboard.writeText(apiKeyCommand)} size="icon-sm" type="button" variant="outline"><CopyIcon /></Button></div></AlertDescription></Alert> : null}<Field data-invalid={urlInvalid}><FieldLabel htmlFor="headscale-url">{mode === "builtin" ? "3. " : "2. "}{copy(language, "控制面 HTTPS 地址", "HTTPS control-plane URL")}</FieldLabel><Input aria-invalid={urlInvalid} id="headscale-url" onBlur={() => setURLTouched(true)} onChange={(event) => setURL(event.target.value)} placeholder="https://headscale.example.com:8443" required type="url" value={url} />{urlInvalid ? <FieldError>{copy(language, "请输入不含账号、查询参数或片段的 HTTPS 地址。", "Enter an HTTPS URL without credentials, query parameters, or fragments.")}</FieldError> : <FieldDescription>{copy(language, "填写浏览器和节点都能访问的地址。", "Use an address reachable by both browsers and nodes.")}</FieldDescription>}</Field><Field data-invalid={keyInvalid}><FieldLabel htmlFor="headscale-key">{mode === "builtin" ? "4. " : "3. "}API Key</FieldLabel><Input aria-invalid={keyInvalid} autoComplete="new-password" id="headscale-key" minLength={integration.secretSet ? undefined : 20} onChange={(event) => setAPIKey(event.target.value.trim())} required={!integration.secretSet} type="password" value={apiKey} /><FieldDescription>{integration.secretSet ? copy(language, "已安全保存。留空会继续使用原 Key；输入新值才会替换。", "Already stored securely. Leave blank to keep it, or enter a new value to replace it.") : copy(language, "至少 20 个字符，只会加密保存。", "At least 20 characters; stored only in encrypted form.")}</FieldDescription>{keyInvalid ? <FieldError>{copy(language, "Key 至少需要 20 个字符。", "The key must contain at least 20 characters.")}</FieldError> : null}</Field>{error ? <FieldError role="alert">{error} {copy(language, "请确认地址可达且 Key 仍有效，然后重试。", "Confirm the address is reachable and the key is valid, then retry.")}</FieldError> : null}</FieldGroup></div><SheetFooter><Button onClick={onClose} type="button" variant="outline">{copy(language, "取消", "Cancel")}</Button><Button disabled={busy || !url || urlInvalid || keyInvalid || !integration.secretSet && apiKey.length < 20} type="submit">{busy ? <Spinner data-icon="inline-start" /> : null}{copy(language, "验证并保存", "Verify and save")}</Button></SheetFooter></form></SheetContent></Sheet>;
}
...
function CloudflareSheet({ integration, language, open, onClose, onSave }: { integration: Integration; language: Language; open: boolean; onClose: () => void; onSave: (input: { accountId: string; zoneId: string; apiToken: string }) => Promise<void> }) {
  ...
  useEffect(() => { if (open) { setAccountID(integration.accountId ?? ""); setZoneID(integration.zoneId ?? ""); setAPIToken(""); setError(""); } }, [open, integration.accountId, integration.zoneId]);
  ...
  const tokenInvalid = apiToken !== "" && apiToken.length < 20;
  return <Sheet onOpenChange={(next) => { if (!next) onClose(); }} open={open}><SheetContent className="sm:max-w-lg"><SheetHeader><SheetTitle>{copy(language, "连接 Cloudflare", "Connect Cloudflare")}</SheetTitle><SheetDescription>{copy(language, "只需填写 Cloudflare 控制台中的三个值。已有 Token 无需重复输入。", "Enter the three values shown in Cloudflare. An existing token need not be re-entered.")}</SheetDescription></SheetHeader><form className="flex flex-1 flex-col" onSubmit={(event) => void submit(event)}><div className="flex-1 px-4"><FieldGroup><Field><FieldLabel htmlFor="cf-account">Account ID</FieldLabel><Input id="cf-account" onChange={(event) => setAccountID(event.target.value.trim())} pattern="[A-Fa-f0-9]{32}" required value={accountId} /><FieldDescription>{copy(language, "Cloudflare 控制台右侧可以复制。", "Copy it from the right side of the Cloudflare dashboard.")}</FieldDescription></Field><Field><FieldLabel htmlFor="cf-zone">Zone ID</FieldLabel><Input id="cf-zone" onChange={(event) => setZoneID(event.target.value.trim())} pattern="[A-Fa-f0-9]{32}" required value={zoneId} /></Field><Field data-invalid={tokenInvalid}><FieldLabel htmlFor="cf-token">API Token</FieldLabel><Input aria-invalid={tokenInvalid} autoComplete="new-password" id="cf-token" minLength={integration.secretSet ? undefined : 20} onChange={(event) => setAPIToken(event.target.value.trim())} required={!integration.secretSet} type="password" value={apiToken} /><FieldDescription>{integration.secretSet ? copy(language, "已安全保存。留空会继续使用原 Token；输入新值才会替换。", "Already stored securely. Leave blank to keep it, or enter a new value to replace it.") : copy(language, "需要 Zone DNS 编辑和 Account Tunnel 编辑权限。", "Needs Zone DNS edit and Account Tunnel edit permissions.")}</FieldDescription>{tokenInvalid ? <FieldError>{copy(language, "Token 至少需要 20 个字符。", "The token must contain at least 20 characters.")}</FieldError> : null}</Field>{error ? <FieldError role="alert">{error} {copy(language, "请检查 ID、Token 权限和网络后重试。", "Check the IDs, token permissions, and network, then retry.")}</FieldError> : null}</FieldGroup></div><SheetFooter><Button onClick={onClose} type="button" variant="outline">{copy(language, "取消", "Cancel")}</Button><Button disabled={busy || !accountId || !zoneId || tokenInvalid || !integration.secretSet && apiToken.length < 20} type="submit">{busy ? <Spinner data-icon="inline-start" /> : null}{copy(language, "验证并保存", "Verify and save")}</Button></SheetFooter></form></SheetContent></Sheet>;
}

...
export function LinkBandwidthSummary({ check, language, unavailable }: { check?: NodeDiagnosticCheck; language: Language; unavailable?: boolean }) {
  ...
  return <span className={`shrink-0 whitespace-nowrap text-right ${valid ? "text-foreground" : failed || unavailable ? "text-destructive" : "text-muted-foreground"}`} title={title} aria-label={title}>
    {valid ? <>↑{valid.uploadMbps.toFixed(1)} ↓{valid.downloadMbps.toFixed(1)} <span className="text-muted-foreground">Mbps</span></> : label}
  </span>;
}

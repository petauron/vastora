...
export function SetupWizard(props: SetupWizardProps) {
  ...
  useEffect(() => {
    writeSetupDraft({ step: step === 1 ? 1 : 2, name, timezone, domainSuffix, mode, agentConnectUrl, headscaleMode, headscaleUrl, publicAddress, gatewayAddress, natConfirmed });
  }, [step, name, timezone, domainSuffix, mode, agentConnectUrl, headscaleMode, headscaleUrl, publicAddress, gatewayAddress, natConfirmed]);
  ...
  return <main className="min-h-svh bg-muted/25 p-4 sm:p-6 md:p-8"><div className="mx-auto flex w-full max-w-5xl flex-col gap-6">
    ...
    <Card>
      ...
      {step === 2 ? <form onSubmit={nextNetwork}><CardHeader><CardTitle>{copy(language, "你准备在哪里使用 Vastora？", "Where will you use Vastora?")}</CardTitle><CardDescription>{copy(language, "选择最符合你的情况，具体网络设置可以稍后调整。", "Choose what best matches your situation. Network details can be changed later.")}</CardDescription></CardHeader><CardContent><FieldGroup>
        ...
        {mode === "headscale" ? <HeadscaleSetup language={language} agentConnectUrl={agentConnectUrl} builtinAvailable={builtinHeadscaleAvailable} headscaleMode={headscaleMode} headscaleUrl={headscaleUrl} headscaleApiKey={headscaleApiKey} cloudflareOAuthAvailable={cloudflareOAuthAvailable} cloudflareConfigured={cloudflareConfigured} cloudflareZone={cloudflareZone} publicAddress={publicAddress} publicAddressCandidates={publicAddressCandidates} gatewayAddress={gatewayAddress} gatewayAddressCandidates={gatewayAddressCandidates} natConfirmed={natConfirmed} automaticallySuggestedMapping={automaticallySuggestedMapping} onAgentConnectUrl={setAgentConnectUrl} onHeadscaleMode={setHeadscaleMode} onHeadscaleUrl={setHeadscaleUrl} onHeadscaleApiKey={setHeadscaleApiKey} onCloudflareConnected={connectedCloudflare} onGatewayAddress={setGatewayAddress} onNATConfirmed={setNATConfirmed} onPublicAddress={(value) => { setPublicAddress(value); if (publicAddressCandidates.some((candidate) => candidate.address === value)) { setGatewayAddress(value); setNATConfirmed(false); } }} /> : null}

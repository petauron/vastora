...
export function SetupWizard(props: SetupWizardProps) {
  ...
  const nextNetwork = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!validCenterURL(agentConnectUrl)) { setError(copy(language, "Center 地址应类似 https://center.example.com:8443，不能包含账号、密码或路径。", "The Center address should look like https://center.example.com:8443 and cannot contain credentials or a path.")); return; }
    if (mode === "headscale") {
      if (!validHeadscaleURL(headscaleUrl)) { setError(copy(language, "私网地址应类似 https://headscale.example.com:8443，不能包含账号、密码或路径。", "The private-network address should look like https://headscale.example.com:8443 and cannot contain credentials or a path.")); return; }
      if (headscaleMode === "builtin" && (!builtinHeadscaleAvailable || new URL(headscaleUrl).port !== "8443" || new URL(agentConnectUrl).port !== "8443")) { setError(copy(language, "自动安装的 Center 和私网地址都需要使用 8443 端口。", "Automatically installed Center and private-network addresses must both use port 8443.")); return; }
  ...
  const connectedCloudflare = (zone: CloudflareZone) => {
    ...
    if (!agentConnectUrl) setAgentConnectUrl(`https://center.${zone.name}:8443`);
    if (!headscaleUrl) setHeadscaleUrl(`https://headscale.${zone.name}:8443`);
  };
  ...
  return <main className="min-h-svh bg-muted/25 p-4 sm:p-6 md:p-8"><div className="mx-auto flex w-full max-w-5xl flex-col gap-6">
    ...
    <Card>
      ...
      {step === 2 ? <form onSubmit={nextNetwork}><CardHeader><CardTitle>{copy(language, "你准备在哪里使用 Vastora？", "Where will you use Vastora?")}</CardTitle><CardDescription>{copy(language, "选择最符合你的情况，具体网络设置可以稍后调整。", "Choose what best matches your situation. Network details can be changed later.")}</CardDescription></CardHeader><CardContent><FieldGroup>
        {!suggestedAgentConnectUrl ? <Alert><ShieldCheckIcon /><AlertTitle>{copy(language, "当前是临时访问地址", "You are using a temporary access address")}</AlertTitle><AlertDescription>{copy(language, "请继续选择使用方式。连接 Cloudflare 后，Vastora 会自动填写服务器能够访问的正式地址。", "Choose how you will use Vastora. After Cloudflare is connected, Vastora fills the permanent address your servers can reach.")}</AlertDescription></Alert> : null}
        <ConnectionMode language={language} mode={mode} onMode={setMode} />
        {mode !== "headscale" ? <Field><FieldLabel htmlFor="setup-center-url">{copy(language, "Center 地址", "Center address")}</FieldLabel><Input id="setup-center-url" onChange={(event) => setAgentConnectUrl(event.target.value)} placeholder="https://center.example.com:8443" required type="url" value={agentConnectUrl} /><FieldDescription>{suggestedAgentConnectUrl ? copy(language, "已自动填写。以后添加的服务器会使用这个地址连接 Center。", "Filled automatically. Servers added later will use this address to reach Center.") : copy(language, "填写服务器能够访问的地址，不要使用当前浏览器里的 127.0.0.1。", "Enter an address servers can reach, not the 127.0.0.1 address in this browser.")}</FieldDescription></Field> : null}
        {mode === "headscale" ? <HeadscaleSetup language={language} agentConnectUrl={agentConnectUrl} builtinAvailable={builtinHeadscaleAvailable} headscaleMode={headscaleMode} headscaleUrl={headscaleUrl} headscaleApiKey={headscaleApiKey} dnsMode={dnsMode} cloudflareOAuthAvailable={cloudflareOAuthAvailable} cloudflareConfigured={cloudflareConfigured} cloudflareZone={cloudflareZone} publicAddress={publicAddress} publicAddressCandidates={publicAddressCandidates} onAgentConnectUrl={setAgentConnectUrl} onHeadscaleMode={setHeadscaleMode} onHeadscaleUrl={setHeadscaleUrl} onHeadscaleApiKey={setHeadscaleApiKey} onDNSMode={setDNSMode} onCloudflareConnected={connectedCloudflare} onPublicAddress={setPublicAddress} /> : null}
...
function HeadscaleSetup(props: HeadscaleSetupProps) {
  ...
  return <section aria-labelledby="secure-connection-title" className="flex flex-col gap-5 border-t pt-6">
    <div><h2 className="text-base font-semibold" id="secure-connection-title">{copy(language, "设置安全连接", "Set up a secure connection")}</h2><p className="mt-1 text-sm leading-6 text-muted-foreground">{copy(language, "Vastora 会自动安装私网服务，并为 Center 和服务器建立加密连接。", "Vastora installs the private-network service and creates an encrypted connection between Center and your servers.")}</p></div>
    {headscaleMode === "builtin" ? <ol className="grid gap-3 sm:grid-cols-3"><SetupOutcome language={language} number="1" titleZh="连接 Cloudflare" titleEn="Connect Cloudflare" descriptionZh="自动配置所需域名" descriptionEn="Configure required domains" /><SetupOutcome language={language} number="2" titleZh="确认两个地址" titleEn="Confirm two addresses" descriptionZh="Center 与私网服务" descriptionEn="Center and private network" /><SetupOutcome language={language} number="3" titleZh="完成安装" titleEn="Finish installation" descriptionZh="自动申请 HTTPS" descriptionEn="Obtain HTTPS automatically" /></ol> : null}
    {headscaleMode === "builtin" && props.dnsMode === "cloudflare" ? <CloudflareOAuthConnect available={props.cloudflareOAuthAvailable} connected={props.cloudflareConfigured} language={language} onConnected={props.onCloudflareConnected} zoneName={props.cloudflareZone} /> : null}
    <div className="grid gap-4 sm:grid-cols-2"><Field><FieldLabel htmlFor="setup-center-url">{copy(language, "Center 地址", "Center address")}</FieldLabel><Input id="setup-center-url" onChange={(event) => props.onAgentConnectUrl(event.target.value)} placeholder="https://center.example.com:8443" required type="url" value={props.agentConnectUrl} /><FieldDescription>{copy(language, "以后添加的服务器会用这个地址连接 Center。", "Servers added later use this address to reach Center.")}</FieldDescription></Field><Field><FieldLabel htmlFor="setup-headscale-url">{copy(language, "私网地址", "Private-network address")}</FieldLabel><Input id="setup-headscale-url" onChange={(event) => props.onHeadscaleUrl(event.target.value)} placeholder={headscaleMode === "builtin" ? "https://headscale.example.com:8443" : "https://headscale.example.com"} required type="url" value={props.headscaleUrl} /><FieldDescription>{copy(language, "服务器先通过这里加入加密私网。", "Servers use this address to join the encrypted private network first.")}</FieldDescription></Field></div>
    ...
    {headscaleMode === "builtin" && props.dnsMode === "manual" ? <Alert><Globe2Icon /><AlertTitle>{copy(language, "需要手动配置两个域名", "Two domains need manual DNS")}</AlertTitle><AlertDescription>{copy(language, "将 Center 和私网地址解析到这台服务器的公网地址，关闭代理，并等待解析生效。", "Point the Center and private-network addresses to this server's public address, disable proxying, and wait for DNS propagation.")}</AlertDescription></Alert> : null}
    <details className="group rounded-2xl border bg-muted/20 p-4"><summary className="flex min-h-6 cursor-pointer list-none items-center gap-3 text-sm font-medium"><Settings2Icon aria-hidden="true" className="size-4 text-muted-foreground" /><span>{copy(language, "高级设置", "Advanced settings")}</span><span className="ml-auto hidden text-xs font-normal text-muted-foreground sm:inline">{copy(language, "端口、Headscale 来源、DNS 和公网地址", "Ports, Headscale source, DNS, and public address")}</span><ChevronDownIcon aria-hidden="true" className="size-4 shrink-0 text-muted-foreground transition-transform group-open:rotate-180" /></summary><FieldGroup className="mt-5 border-t pt-5"><Field><FieldLabel htmlFor="setup-headscale-mode">{copy(language, "私网服务", "Private-network service")}</FieldLabel><NativeSelect id="setup-headscale-mode" onChange={(event) => props.onHeadscaleMode(event.target.value as "builtin" | "external")} value={headscaleMode}><option disabled={!props.builtinAvailable} value="builtin">{copy(language, "由 Vastora 自动安装（推荐）", "Install automatically with Vastora (recommended)")}</option><option value="external">{copy(language, "连接已有 Headscale", "Connect an existing Headscale")}</option></NativeSelect>{headscaleMode === "builtin" ? <FieldDescription>{copy(language, "固定使用 8443，不占用应用的 443；安装时需要开放 80。", "Uses port 8443 so application port 443 stays free; port 80 is needed during installation.")}</FieldDescription> : null}</Field>{headscaleMode === "builtin" ? <Field><FieldLabel htmlFor="setup-dns-mode">{copy(language, "域名配置", "Domain setup")}</FieldLabel><NativeSelect id="setup-dns-mode" onChange={(event) => props.onDNSMode(event.target.value as "cloudflare" | "manual")} value={props.dnsMode}><option disabled={!automaticAvailable} value="cloudflare">{copy(language, "Cloudflare 自动配置（推荐）", "Configure automatically with Cloudflare (recommended)")}</option><option value="manual">{copy(language, "我会手动配置 DNS", "I will configure DNS manually")}</option></NativeSelect>{!automaticAvailable ? <FieldDescription>{props.publicAddressCandidates.length === 0 ? copy(language, "没有发现服务器公网地址，因此暂时只能手动配置。", "No server public address was found, so DNS must be configured manually.") : copy(language, "当前 Center 版本未启用 Cloudflare 登录。", "Cloudflare sign-in is not enabled in this Center build.")}</FieldDescription> : null}</Field> : null}{headscaleMode === "builtin" && props.dnsMode === "cloudflare" ? <Field><FieldLabel htmlFor="setup-public-address">{copy(language, "服务器公网地址", "Server public address")}</FieldLabel><NativeSelect id="setup-public-address" onChange={(event) => props.onPublicAddress(event.target.value)} value={props.publicAddress}>{props.publicAddressCandidates.map((candidate) => <option key={candidate.address} value={candidate.address}>{candidate.address} — {candidate.interface}</option>)}</NativeSelect><FieldDescription>{copy(language, "只显示真实配置在服务器网卡上的地址。", "Only addresses assigned to a server interface are shown.")}</FieldDescription></Field> : null}</FieldGroup></details>
  </section>;
}

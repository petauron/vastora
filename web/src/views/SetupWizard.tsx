...
export function SetupWizard({ language, suggestedAgentConnectUrl, builtinHeadscaleAvailable, onLanguage, onComplete }: SetupWizardProps) {
  ...
  const nextNetwork = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (mode === "headscale") {
      if (!validHeadscaleURL(headscaleUrl)) {
        setError(copy(language, "Headscale 地址必须是没有路径的 HTTPS URL。", "The Headscale address must be an HTTPS URL without a path."));
        return;
      }
      if (headscaleMode === "builtin" && (!builtinHeadscaleAvailable || new URL(headscaleUrl).port !== "8443" || new URL(agentConnectUrl).port !== "8443")) {
        setError(copy(language, "内置 Headscale 需要当前安装包的部署助手，并固定使用 8443 端口。", "Built-in Headscale needs the bundled deployment helper and must use port 8443."));
        return;
      }
      if (headscaleMode === "external" && headscaleApiKey.trim().length < 20) {
        setError(copy(language, "已有 Headscale 需要有效的 API Key。", "An existing Headscale server requires a valid API key."));
        return;
      }
    }
    setStep(3);
  ...
  return <main className="min-h-svh bg-muted/35 p-5 md:p-8"><div className="mx-auto flex w-full max-w-2xl flex-col gap-6"><div className="flex items-center justify-between"><Brand /><Button aria-label={copy(language, "切换语言", "Change language")} onClick={() => onLanguage(language === "zh-CN" ? "en" : "zh-CN")} size="icon" variant="ghost"><LanguagesIcon /></Button></div><SetupProgress language={language} step={step} /><Card>
    ...
    {step === 2 ? <form onSubmit={nextNetwork}><CardHeader><CardTitle>{copy(language, "节点如何连接 Center？", "How will nodes reach Center?")}</CardTitle><CardDescription>{copy(language, "这是添加节点时的默认接入方式。以后仍可以同时启用局域网、Headscale 和公网能力。", "This is the default joining path. LAN, Headscale, and public capabilities can still be enabled together later.")}</CardDescription></CardHeader><CardContent><FieldGroup>{!suggestedAgentConnectUrl ? <Alert><ShieldCheckIcon /><AlertTitle>{copy(language, "安装向导通过 SSH 隧道打开", "Setup is open through an SSH tunnel")}</AlertTitle><AlertDescription>{copy(language, "下面填写 Agent 以后实际能访问的 Center 地址，不要填写本机浏览器中的 127.0.0.1:18082。", "Enter the Center address that Agents will actually reach later, not 127.0.0.1:18082 from this browser.")}</AlertDescription></Alert> : null}<FieldSet><FieldLegend>{copy(language, "主要接入环境", "Primary connection environment")}</FieldLegend><div className="grid gap-3">{connectionOptions.map((option) => { const Icon = option.icon; return <label className="flex min-h-20 cursor-pointer items-start gap-3 rounded-xl border p-4 has-checked:border-primary has-checked:bg-primary/5" key={option.mode}><input checked={mode === option.mode} className="mt-1" name="connection-mode" onChange={() => setMode(option.mode)} type="radio" value={option.mode} /><Icon aria-hidden="true" className="mt-0.5 size-5 shrink-0" /><span className="min-w-0 flex-1"><span className="flex items-center gap-2 text-sm font-medium">{copy(language, option.zh, option.en)}{option.mode === "headscale" ? <Badge variant="secondary">{copy(language, "跨站点推荐", "Recommended across locations")}</Badge> : null}</span><span className="mt-1 block text-xs leading-5 text-muted-foreground">{copy(language, option.descriptionZh, option.descriptionEn)}</span></span></label>; })}</div></FieldSet><Field><FieldLabel htmlFor="setup-center-url">{copy(language, "Agent 连接地址", "Agent connection address")}</FieldLabel><Input id="setup-center-url" onChange={(event) => setAgentConnectUrl(event.target.value)} placeholder="https://center.example.com:8443" required type="url" value={agentConnectUrl} /><FieldDescription>{suggestedAgentConnectUrl ? copy(language, "已使用安装 Center 时配置的地址。节点必须能访问它。", "Filled from the Center installation. Every node must be able to reach it.") : copy(language, "填写目标节点能够访问的 Center HTTPS 地址。内置网关使用 8443，不占用应用的 443。", "Enter the Center HTTPS address reachable from target nodes. The bundled gateway uses 8443 and leaves application port 443 free.")}</FieldDescription></Field>{mode === "headscale" ? <div className="rounded-xl border bg-muted/25 p-4"><FieldGroup><Field><FieldLabel htmlFor="setup-headscale-mode">{copy(language, "Headscale 来源", "Headscale source")}</FieldLabel><NativeSelect id="setup-headscale-mode" onChange={(event) => setHeadscaleMode(event.target.value as "builtin" | "external")} value={headscaleMode}><option disabled={!builtinHeadscaleAvailable} value="builtin">{copy(language, "自动安装到 Center 服务器", "Install automatically on the Center server")}</option><option value="external">{copy(language, "连接已有 Headscale", "Connect an existing Headscale")}</option></NativeSelect></Field>{headscaleMode === "builtin" ? <Alert><ShieldCheckIcon /><AlertTitle>{copy(language, "向导会自动完成安装", "The wizard installs it automatically")}</AlertTitle><AlertDescription>{copy(language, "请先让 Center 与 Headscale 两个域名都解析到这台服务器。Vastora 会安装固定版本、申请 HTTPS 证书并安全保存自动创建的 API Key。需要开放 80 和 8443。", "Point the Center and Headscale hostnames to this server first. Vastora installs fixed versions, obtains HTTPS certificates, and securely stores the generated API key. Ports 80 and 8443 must be open.")}</AlertDescription></Alert> : null}<Field><FieldLabel htmlFor="setup-headscale-url">Headscale URL</FieldLabel><Input id="setup-headscale-url" onChange={(event) => setHeadscaleUrl(event.target.value)} placeholder={headscaleMode === "builtin" ? "https://headscale.example.com:8443" : "https://headscale.example.com"} required type="url" value={headscaleUrl} /><FieldDescription>{headscaleMode === "builtin" ? copy(language, "必须使用独立域名和 8443 端口。", "Use a separate hostname and port 8443.") : copy(language, "填写浏览器和节点都能访问的地址。", "Use an address reachable by browsers and nodes.")}</FieldDescription></Field>{headscaleMode === "external" ? <Field><FieldLabel htmlFor="setup-headscale-key">API Key</FieldLabel><Input autoComplete="off" id="setup-headscale-key" minLength={20} onChange={(event) => setHeadscaleApiKey(event.target.value)} required type="password" value={headscaleApiKey} /><FieldDescription>{copy(language, "只会加密保存，不会再次显示。", "Stored only in encrypted form and never shown again.")}</FieldDescription></Field> : null}</FieldGroup></div> : null}{error ? <FieldError>{error}</FieldError> : null}</FieldGroup></CardContent><CardFooter className="justify-between"><Button onClick={() => { setError(""); setStep(1); }} type="button" variant="outline">{copy(language, "返回", "Back")}</Button><Button type="submit">{copy(language, "检查配置", "Review")}</Button></CardFooter></form> : null}
    {step === 3 ? <><CardHeader><CardTitle>{copy(language, "确认首次设置", "Review initial setup")}</CardTitle><CardDescription>{copy(language, "完成后将直接进入“添加节点”。位置可在主页修改，单个节点也可覆盖连接地址。", "After finishing, Vastora opens Add node. Locations remain editable, and individual nodes can override the address.")}</CardDescription></CardHeader><CardContent className="flex flex-col gap-5"><Alert><CheckIcon /><AlertTitle>{copy(language, "已准备好", "Ready to finish")}</AlertTitle><AlertDescription>{mode === "headscale" && headscaleMode === "builtin" ? copy(language, "下一步会安装 Headscale 与 HTTPS 网关，通常需要一到三分钟。不会安装任何应用，也不会占用 443。", "The next step installs Headscale and its HTTPS gateway and usually takes one to three minutes. No apps are installed and port 443 remains free.") : copy(language, "不会自动安装应用或开放公网端口。", "No apps are installed and no public ports are opened automatically.")}</AlertDescription></Alert><dl className="grid gap-4 rounded-xl border p-4 text-sm sm:grid-cols-2"><Summary icon={MapPinIcon} label={copy(language, "位置", "Location")} value={name} /><Summary icon={NetworkIcon} label={copy(language, "接入环境", "Connection")} value={copy(language, selected.zh, selected.en)} /><Summary icon={Globe2Icon} label={copy(language, "Agent 连接地址", "Agent address")} value={agentConnectUrl} wide />{mode === "headscale" ? <Summary icon={ShieldCheckIcon} label="Headscale" value={headscaleMode === "builtin" ? copy(language, `自动安装 · ${headscaleUrl}`, `Automatic install · ${headscaleUrl}`) : headscaleUrl} wide /> : null}<Summary icon={HouseIcon} label={copy(language, "时区", "Time zone")} value={timezone} /><Summary icon={Globe2Icon} label={copy(language, "默认域名", "Default domain")} value={domainSuffix || copy(language, "未设置", "Not set")} /></dl>{busy && mode === "headscale" && headscaleMode === "builtin" ? <Alert><Spinner /><AlertTitle>{copy(language, "正在安装 Headscale", "Installing Headscale")}</AlertTitle><AlertDescription>{copy(language, "正在下载固定版本、启动服务、申请 HTTPS 证书并验证连接。请保持此页面打开。", "Downloading fixed versions, starting services, obtaining HTTPS certificates, and verifying connectivity. Keep this page open.")}</AlertDescription></Alert> : null}{error ? <FieldError>{error}</FieldError> : null}</CardContent><CardFooter className="justify-between"><Button disabled={busy} onClick={() => { setError(""); setStep(2); }} variant="outline">{copy(language, "返回", "Back")}</Button><Button disabled={busy} onClick={() => void finish()}>{busy ? <Spinner data-icon="inline-start" /> : null}{busy && mode === "headscale" && headscaleMode === "builtin" ? copy(language, "正在安装…", "Installing…") : copy(language, "完成并添加节点", "Finish and add a node")}</Button></CardFooter></> : null}
  </Card></div></main>;
}
...
function validHeadscaleURL(value: string) {
  try {

...
function HeadscaleSetup(props: HeadscaleSetupProps) {
  ...
  return <section aria-labelledby="secure-connection-title" className="flex flex-col gap-5 border-t pt-6">
    ...
    <div className="grid gap-4 sm:grid-cols-2"><Field><FieldLabel htmlFor="setup-center-url">{copy(language, "Center 地址", "Center address")}</FieldLabel><Input id="setup-center-url" onChange={(event) => props.onAgentConnectUrl(event.target.value)} placeholder="https://center.example.com" required type="url" value={props.agentConnectUrl} /><FieldDescription>{copy(language, "以后添加的服务器会用这个地址连接 Center。", "Servers added later use this address to reach Center.")}</FieldDescription></Field><Field><FieldLabel htmlFor="setup-headscale-url">{copy(language, "私网地址", "Private-network address")}</FieldLabel><Input id="setup-headscale-url" onChange={(event) => props.onHeadscaleUrl(event.target.value)} placeholder="https://headscale.example.com" required type="url" value={props.headscaleUrl} /><FieldDescription>{copy(language, "服务器先通过这里加入加密私网。", "Servers use this address to join the encrypted private network first.")}</FieldDescription></Field></div>

...
export type AgentView = {
  ...
  siteId: string;
  roles: string[];
  capabilities: { docker: boolean; gateway: boolean; tunnel: boolean; metrics: boolean; logs: boolean; ipQuality?: boolean; networkDiagnostics?: boolean; returnRoute?: boolean; bandwidthDiagnostics?: boolean; hostProfile?: boolean };
...
export type NodeProtocols = { vless: boolean; hy2: boolean; commandId?: string; state: string };
...
export type RealitySecurityCheckItem = { kind: "expected_fallback" | "openai_sni" | "cloudflare_sni" | "random_sni" | "no_sni"; status: "passed" | "failed" | "inconclusive"; reason: "expected_fallback_verified" | "expected_fallback_unavailable" | "unauthorized_destination_rejected" | "unauthorized_destination_reached" | "local_tls_termination" | "probe_timeout" | "probe_interrupted" };
export type RealitySecurityCheck = { status: "safe" | "affected" | "inconclusive"; scope: "remote" | "same_host"; checks: RealitySecurityCheckItem[]; checkedAt: string };
export type Publication = { id: string; serviceId: string; kind: PublicationKind; ingress: PublicationIngress; hostname: string; sniHostname?: string; dnsProvider: "manual" | "cloudflare" | "headscale"; dnsRecordId?: string; dnsRecord?: DNSRecordInstruction; tlsEnabled: boolean; certificateExpiresAt?: string; desiredRevision: number; appliedRevision: number; status: "pending" | "applying" | "ready" | "degraded" | "failed" | "stopped"; lastError?: string; actionRequired?: boolean; accessUrl?: string; securityCheck?: RealitySecurityCheck; createdAt: string; updatedAt: string };
...
export type Deployment = {
  ...
  applicationId?: string;
  oneTimeCredentials?: { username?: string; password?: string; setupToken?: string };

...
export const api = {
  ...
  deployments: (signal?: AbortSignal) => request<{ deployments: Deployment[] }>("/api/v1/deployments", { signal }),
	...
	createRealityCommand: (input: { applicationId: string; regionCode: string; name: string; clientName?: string; gatewayNodeId: string; hostname: string; dnsProvider: "manual" | "cloudflare"; targetHost: string; serverName: string; inboundTotalBytes: number; inboundResetDays: number; clientTotalBytes?: number; clientResetDays?: number; clientExpiryTime?: number }) => request<ApplicationCommand>("/api/v1/application-commands/reality", { method: "POST", body: JSON.stringify(input) }),
  ...
  confirmNetworkProfile: (agentId: string, profile: NetworkProfile) => request<NetworkProfile>(`/api/v1/agents/${encodeURIComponent(agentId)}/network-profile`, { method: "PUT", body: JSON.stringify(profile) }),
	detectAgentPublicEntry: (agentId: string) => request<{ status: "ready"; publicAddress: string; gatewayAddress: string; mode: "direct" | "nat"; profile: NetworkProfile }>(`/api/v1/agents/${encodeURIComponent(agentId)}/public-entry/detect`, { method: "POST", body: "{}" }),

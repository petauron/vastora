...
export const api = {
  ...
  decideAssistantProposal: (id: string, decision: "approve" | "reject", digest: string) => request<AssistantProposal>(`/api/v1/assistant/proposals/${encodeURIComponent(id)}/${decision}`, { method: "POST", body: JSON.stringify({ digest }) }),
	applyAssistantProposal: (id: string, digest: string) => request<AssistantExecution>(`/api/v1/assistant/proposals/${encodeURIComponent(id)}/apply`, { method: "POST", body: JSON.stringify({ digest }) }),
  ...
  createDeployment: (agentId: string, appKey: string, config: Record<string, string | boolean | number>, operation: Deployment["operation"] = "install", deleteData = false, role?: "master" | "worker", registryCredentialId?: string, operationKey?: string, authorizedCapabilities?: string[], packageIdentity?: { packageRevision: number; manifestSha256: string }) => request<Deployment>("/api/v1/deployments", { method: "POST", headers: operationKey ? { "Idempotency-Key": operationKey } : undefined, body: JSON.stringify({ agentId, appKey, config, operation, deleteData, role, registryCredentialId, authorizedCapabilities, ...packageIdentity }) }),
  revealDeploymentCredentials: (id: string, operationKey: string) => request<{ username?: string; password?: string; setupToken?: string }>(`/api/v1/deployments/${encodeURIComponent(id)}/credentials/reveal`, { method: "POST", headers: { "Idempotency-Key": operationKey }, body: "{}" }),
  ...
  confirmNetworkProfile: (agentId: string, profile: NetworkProfile) => request<NetworkProfile>(`/api/v1/agents/${encodeURIComponent(agentId)}/network-profile`, { method: "PUT", body: JSON.stringify(profile) }),
	createPublication: (input: CreatePublicationInput) => request<Publication>("/api/v1/publications", { method: "POST", body: JSON.stringify(input) }),
  ...
  checkRealitySecurity: (id: string) => request<RealitySecurityCheck>(`/api/v1/publications/${encodeURIComponent(id)}/security-check`, { method: "POST", body: "{}" }),
  ...
  configureHeadscale: (input: { mode: "builtin" | "external"; url: string; apiKey?: string; dnsPolicy?: "system" | "custom"; dnsResolvers?: string[] }) => request<Integration>("/api/v1/integrations/headscale", { method: "PUT", body: JSON.stringify(input) }),

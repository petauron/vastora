...
export function App() {
  ...
  if (phase === "setup-admin") return <CredentialPage language={language} mode="setup" onLanguage={setLanguage} onSubmit={async (username, password) => { await api.setupAdmin(username, password); setSetupStatus((current) => ({ administratorConfigured: true, onboardingComplete: false, suggestedAgentConnectUrl: current?.suggestedAgentConnectUrl ?? "" })); setPhase("setup-wizard"); }} />;
  if (phase === "setup-wizard") return <SetupWizard language={language} onLanguage={setLanguage} suggestedAgentConnectUrl={setupStatus?.suggestedAgentConnectUrl ?? ""} onComplete={async (input) => { await api.completeSetup(input); setSetupStatus((current) => ({ administratorConfigured: true, onboardingComplete: true, suggestedAgentConnectUrl: current?.suggestedAgentConnectUrl ?? "" })); try { await loadDashboard(); setScreen("nodes"); setAddFirstNode(true); setPhase("ready"); } catch (error) { setNotice({ message: error instanceof Error ? error.message : "Center unavailable", error: true }); setPhase("unavailable"); } }} />;

...
func TestServerExposesOnlyTheFixedHeadscaleOperation(t *testing.T) {
	...
	payload, _ := json.Marshal(deployapi.HeadscaleInstallRequest{CenterURL: "https://center.example.com:8443", HeadscaleURL: "https://headscale.example.com:8443"})
	request := httptest.NewRequest(http.MethodPost, "/v1/headscale/install", bytes.NewReader(payload))
	request.Header.Set("Content-Type", "application/json")
	response := httptest.NewRecorder()
	handler.ServeHTTP(response, request)
	if response.Code != http.StatusOK || installer.input.HeadscaleURL != "https://headscale.example.com:8443" {

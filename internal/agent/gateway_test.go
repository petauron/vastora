...
func TestSystemGatewayProtectionIsRecoveredFromCompleteDesiredState(t *testing.T) {
	desired := gateway.DesiredState{
		...
		Routes: []gateway.Route{
			...
			{ID: "system-docker-bootstrap", Hostname: "headscale.example.test", Path: "/install/docker.sh", Protocol: "http", Upstreams: []gateway.Upstream{{Address: "vastora-center", Port: 8080}}, TLSEnabled: true, ListenerKind: "public", System: true},
...
func TestProtectedSystemGatewayAcceptsCompleteSystemRoutes(t *testing.T) {
	...
	desired.Routes = append(desired.Routes, gateway.Route{ID: "system-docker-bootstrap", Hostname: "headscale.example.test", Path: "/install/docker.sh", Protocol: "http", Upstreams: []gateway.Upstream{{Address: "127.0.0.1", Port: 8080}}, TLSEnabled: true, ListenerKind: "public", System: true})

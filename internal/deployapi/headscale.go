...
func (client *Client) InstallHeadscale(ctx context.Context, input HeadscaleInstallRequest) (HeadscaleInstallResult, error) {
	...
	request, err := http.NewRequestWithContext(ctx, http.MethodPost, "http://deployer/v1/headscale/install", bytes.NewReader(payload))
	if err != nil {
		return HeadscaleInstallResult{}, err
	}
	...
	if err != nil {
		return HeadscaleInstallResult{}, fmt.Errorf("center: contact deployment helper: %w", err)
	}
	defer response.Body.Close()
	body, err := io.ReadAll(io.LimitReader(response.Body, 1<<20))
	if err != nil {
		return HeadscaleInstallResult{}, fmt.Errorf("center: read deployment helper response: %w", err)
	}
	if response.StatusCode != http.StatusOK {
		...
		if json.Unmarshal(body, &failure) == nil && failure.Error != "" {
			return HeadscaleInstallResult{}, errors.New(failure.Error)
		}
		return HeadscaleInstallResult{}, fmt.Errorf("center: deployment helper returned HTTP %d", response.StatusCode)
	}

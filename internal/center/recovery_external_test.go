...
func TestExternalVolumeEvidenceCannotHideMissingOrChangedCoverage(t *testing.T) {
	...
	node := enrollOrchestrationNode(t, store, "recovery-volumes", NodeCapabilities{Docker: true}, []networking.Candidate{{Address: "10.0.0.81", Interface: "eth0"}}, networking.Profile{ServiceAddress: "10.0.0.81", LANAddress: "10.0.0.81", EnabledKinds: []string{networking.KindLAN}})
	now := store.now().UTC()

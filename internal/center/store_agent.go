...
func (s *Store) RecordAgentHeartbeat(ctx context.Context, id, credential string, heartbeat NodeHeartbeat) error {
	...
	recoveredPublications, err := publicationsAfterNetworkRecovery(ctx, tx, now)
	if err != nil {
		return err
	}
	if err := tx.Commit(); err != nil {
	...
	for _, target := range recoveredPublications {
		s.schedulePublicationVerification(target.id, target.revision)
	}
	if heartbeat.Startup {
		// Recovery has its own durable action_required state and minute retry.
		// It must not turn an already-recorded management heartbeat into an
		// error or stall enrollment while a different service needs repair.
		s.startBackground(func() { _ = s.startRealityGuardHardening(s.backgroundCtx) })
	}
...
func invalidateUnusableNetworkProfile(ctx context.Context, tx *sql.Tx, agentID string) error {
	...
	if profile == nil {
		retained, err := recoverableNetworkProfile(ctx, tx, agentID)
		if err != nil || retained == nil {
			return err
		}
		if networking.ValidateProfile(candidates, *retained) != nil {
			return nil
		}
		if retained.DirectPublic {
			egress, err := agentPublicEgress(ctx, tx, agentID)
			if err != nil {
				return err
			}
			if egress == nil || egress.Address != retained.PublicAddress || egress.BindAddress != retained.PublicBindAddress || egress.Mode != retained.PublicMode {
				return nil
			}
			retained.PublicVerifiedAt = egress.ObservedAt
		}
		for _, candidate := range candidates {
			if candidate.ObservedAt.After(retained.CandidateObserved) {
				retained.CandidateObserved = candidate.ObservedAt
			}
		}
		return saveNetworkProfile(ctx, tx, agentID, *retained)
	}
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO agent_network_profile_recovery(agent_id, public_key, profile_json)
		SELECT id, x25519_public_key, ? FROM agents WHERE id = ?
		...
		return fmt.Errorf("center: retain confirmed Agent network profile: %w", err)
	}

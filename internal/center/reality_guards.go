...
func (s *Store) completeRealityHardenCommand(ctx context.Context, tx *sql.Tx, taskID, agentID, applicationID, gatewayID string, inputJSON []byte, succeeded bool, taskError string, rawResult json.RawMessage) error {
	...
	if succeeded {
		if len(rawResult) == 0 || json.Unmarshal(rawResult, &envelope) != nil || envelope.ApplicationCommand == nil {
			...
			if result.Action != "harden" || result.InboundID != input.InboundID || result.Listen != input.TargetAddress || result.Port != centerThreeXUIRealityPort || (result.InboundTag != input.InboundTag && result.InboundTag != expectedInboundTag) || result.TargetHost != input.TargetHost || result.ServerName != input.ServerName || !validRealityTargetHostname(result.TargetHost) || !validRealityTargetHostname(result.ServerName) || net.ParseIP(result.TargetIP) == nil || result.NodeASN <= 0 || result.TargetASN <= 0 || result.NodeASN != result.TargetASN || result.CDNProvider != "" || !result.TLS13 || !result.X25519 || !result.HTTP2 || !result.CertificateValid || result.CompanionInboundID < 1 || (result.CompanionTag != input.InboundTag+"-guard" && result.CompanionTag != expectedCompanionTag) || result.CompanionPort != expectedGuardPort || result.GuardStatus != "ready" || !result.ProxyProtocol {
				succeeded = false

...
func verifyCenterEncryptedState(ctx context.Context, db *sql.DB, key []byte) error {
	...
	queries := []encryptedReferenceQuery{
		...
		{"assistant_model_providers", []string{"id", "api_key_secret_id"}, `SELECT api_key_secret_id, 'assistant-provider:default' FROM assistant_model_providers`},
		...
		{"landing_client_grants", []string{"id", "credential_secret_id"}, `SELECT credential_secret_id, 'landing-credential:' || id FROM landing_client_grants`},
		{"landing_client_grants", []string{"id", "material_secret_id"}, `SELECT material_secret_id, 'landing-material:' || id FROM landing_client_grants WHERE material_secret_id IS NOT NULL`},

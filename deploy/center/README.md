...
3. If you choose built-in Headscale, enter the two DNS names that already point
   to this server. The wizard installs Headscale, creates its API key, and starts
   the HTTPS gateway automatically on ports 80 and 443. Center and Headscale use
   separate hostnames on the same standard HTTPS gateway. If this server also
   runs Agent, Agent adopts that Caddy instance instead of installing a second
   one. HAProxy is added only when a raw TLS service must share public port 443.

...
- Center 和目标 Agent 都需要包含本功能的版本；过渡控制端需要支持原生 HY2。
...
- 新增 HY2 只关联已经分配给该节点的客户端；凭据由过渡控制端在 Vastora
  任务内生成，并写入加密 worker journal，不能从 worker 反向覆盖权威状态。
...
- 启停 HY2 会生成并验证新的完整 Xray revision，然后短暂重启该节点的
  Xray worker；应用升级保留显式启用的 UDP 映射。
